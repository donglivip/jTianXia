...
Page({
   ...
   * areas当前被选中的区
   * 
   * areaID地区ID字符串，传服务器保存，以此确定区域
   */
  data: {
    //user_openid:this.globalData.user_openid,
    menuType: 0,
    begin: null,
    status: 1,
    end: null,
  ...
  onLoad: function (options) {
    ...
    // 默认联动显示北京
    var id = 0//this.data.provinces[0].id
  ...
  formSubmit: function (e) {
    ...
    var fbc1_status
    if (typeof(tj_data['fbc1_status']) == "string"){
      fbc1_status = 1
    }
    var zt = tj_data['fbc1_status']
    //console.log(zt)
    ...
    if (this.data.radio) {
      ...
      tj_data['fbc1_status'] = fbc1_status
      ...
      var that = this
      wx.request({
        url: app.globalData.apiUrl + '/api/fbapi/add_ershou',
        method: 'post',
        data: tj_data,
        header: {
          'content-type': 'application/json'

...
Page({
        ...
        if (img_count + 1 > 9) {
          ...
          wx.uploadFile({
            ...
            success(res2) {
              ...
              console.log(that.data.upload_img_path2)
              wx.showToast({
                title: msg['info'],
                icon: 'loading',//图标，支持"success"、"loading"
                mask: false,  //是否显示透明蒙层，防止触摸穿透，默认：false
  ...
  formSubmit: function (e) {
    ...
    if (this.data.radio) {
      ...
      var fb_info = {
        "5": ['add_weixiu', 'fbc5_name', 'fbc5_address'],
        "6": ['add_jixie', 'fbc6_name', 'fbc6_address'],
        "7": ['add_wujin', 'fbc7_name', 'fbc7_address'],
        "8": ['add_jiancai', 'fbc8_name', 'fbc8_address'],
        "9": ['add_shigong', 'fbc9_name', 'fbc9_address'],
      }

      tj_data[fb_info[fb_category_id][1]] = tj_data['fbc_name']
      tj_data[fb_info[fb_category_id][2]] = tj_data['fbc_address']
      ...
      var that = this
      wx.request({
        url: app.globalData.apiUrl + '/api/fbapi/' + fb_info[fb_category_id][0],
        method: 'post',
        data: tj_data,
        header: {
          'content-type': 'application/json'
